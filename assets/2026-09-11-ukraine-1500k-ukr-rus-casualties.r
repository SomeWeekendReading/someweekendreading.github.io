...
doit <- function(## Inputs
  ...
  withTranscript(dataDir, resultsDir,                  # Capture stdout to transcript
    ...
    ## Addendum 2026-Apr-02: 1300k casualties
    lateCasualties <- rbind(lateCasualties,            #
                            transform(data.frame(Date     = as.Date(c("2026-04-02")),
                                                 Soldiers = c(1300030)),
                                      DayNum = as.integer(difftime(Date, min(ukrRusCasualties$"Date"),
                                                                   units = "days"))))
    compareLateDay(ukrRusCasualties, lateCasualties, "DayNum1300k", resultsDir, regressFile)

    ## Addendum 2026-Jun-28: 1400k casualties
    lateCasualties <- rbind(lateCasualties,            #
                            transform(data.frame(Date     = as.Date(c("2026-06-28")),
                                                 Soldiers = c(1400970)),
                                      DayNum = as.integer(difftime(Date, min(ukrRusCasualties$"Date"),
                                                                   units = "days"))))
    compareLateDay(ukrRusCasualties, lateCasualties, "DayNum1400k", resultsDir, regressFile)

    ## Addendum 2026-Sep-09: 1500k casualties
    lateCasualties <- rbind(lateCasualties,            #
                            transform(data.frame(Date     = as.Date(c("2026-09-09")),
                                                 Soldiers = c(1500870)),
                                      DayNum = as.integer(difftime(Date, min(ukrRusCasualties$"Date"),
                                                                   units = "days"))))
    compareLateDay(ukrRusCasualties, lateCasualties, "DayNum1500k", resultsDir, regressFile)
...
## > rm("updatedData", "crossvalidatedSegmentFits"); doit2("russian-casualties-in-ukraine-updated-2026-09-11.tsv")
  ...
  withTranscript(dataDir, resultsDir,                  # Capture stdout to transcript
    ...
    maybeAssign("updatedData",                         #  should have 3 columns:
                function() {                           #  DayNum, Date, Soldiers
                  ...
                  ## In an apparent new behavior (2026-Apr-02, R 4.4.2) just transforming on a
                  ## new column of 1 : kFolds no longer works if kFolds is not an exact divisor
                  ## of the number of rows.  I wonder what else that breaks!
                  ##
                  ## So we resort to trickery with row numbers.
                  before <- subset(updatedData, subset = DayNum <  updateDay)
                  after  <- subset(updatedData, subset = DayNum >= updateDay)
                  updatedData <-                       # Assign crossvalidation folds
                    rbind(transform(before, Fold = (1 : nrow(before) - 1) %% kFolds + 1),
                          transform(after,  Fold = (1 : nrow(after)  - 1) %% kFolds + 1))
