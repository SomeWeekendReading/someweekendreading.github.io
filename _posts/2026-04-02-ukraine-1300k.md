...
know, we have been expressing&hellip; _strong opinions_&hellip; about Russia's invasion
...
realize the true model could be kink-free and what we see could be due to chance alone.  If this
gives a very small $p$-value, then we accept that the kink is a worthwhile addition, and
probably real (i.e., would reproduce in another experiment).  
...
model.  (This is the out-of-sample RMS error on a test set under crossvalidation, _q.v._)
Note that the RMS error of the kinked model is _3.48 times lower_ than the simple 
...
here.  Just look at the 95% confidence limits on the slope estimates in the middle of the
wheat-colored box on the lower right of the plot: they are not just
disjoint, but _far apart._  This means there is almost no chance that the slopes are
actually equal, and we just saw them be different by chance.  The slope difference is real
(statistically significant Davies test) and large (effect size is 3.5x lower RMS error under
crossvalidation).  
...
That's why we statisticians invented
